...
pub(crate) fn generic_image_to_oklab<const IMAGE: u8>(
    ...
    for _ in 0..height as usize {
        let dst_ptr = unsafe { (dst.as_mut_ptr() as *mut u8).add(oklab_offset) as *mut u16 };
        ...
        for x in 0..width as usize {
            ...
            unsafe {
                *new_slice.get_unchecked_mut(x) = (oklab.l * full_scale).round().min(scale) as u16;
                // Just for storing in u16 adding 500 to subtract 500 after to keep values in positive range
                *color_planes.get_unchecked_mut(color_planes_offset + cx + 0) = oklab.a;
                *color_planes.get_unchecked_mut(color_planes_offset + cx + 1) = oklab.b;
        ...
        oklab_offset += dst_stride as usize;
